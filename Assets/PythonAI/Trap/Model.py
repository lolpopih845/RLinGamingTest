...
class ActorNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256,temperature=1.0, chkpt_dir=os.getcwd()+"/Trap/model"):
        super(ActorNetwork, self).__init__()

        self.checkpoint_file = os.path.join(chkpt_dir, 'ActorBoi')
        self.fc1 = nn.Linear(input_dims, fc1_dims)
        self.fc2 = nn.Linear(fc1_dims, fc2_dims)
        self.head = nn.Linear(fc2_dims, 2*21) 
    ...
    def forward(self, state):
        ...
        rest_output_probs = F.log_softmax(rest_output_logits.view(-1, 2, 21), dim=-1)
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256,
            chkpt_dir=os.getcwd()+"/Trap/model"):
